...
# 테스트 이미지 경로
test_image_path = os.path.join(os.path.dirname(script_dir), 'testImage', 'test2.png')
...
# 결과 저장
output_path = os.path.join(script_dir, 'test_result_tta.jpg')
...
try:
    ...
    plt.tight_layout()
    plot_path = os.path.join(script_dir, 'test_result_tta_plot.png')
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    print(f"[SAVED] Visualization saved to: {plot_path}")
    # plt.show()  # GUI 환경에서만 주석 해제
except Exception as e:
    print(f"[INFO] Matplotlib display skipped: {e}")
